...
export default function PatientJ0() {
    ...
    const loadResponses = async () => {
        ...
        setLoading(true);
        const data = await getResponses(parseInt(resolvedPatientId), 'J0');
        setResponses(data);
        setLoading(false);
    };
    ...
    const handleChange = async (itemId, value) => {
        ...
        if (resolvedPatientId) {
            await saveResponse(parseInt(resolvedPatientId), 'J0', itemId, value, false);
        }
    };
    ...
    const handleSubmit = async () => {
        setSaving(true);
        if (resolvedPatientId) {
            await markScreenCompleted(parseInt(resolvedPatientId), 'J0');
        }
        setSaving(false);
        if (isTokenMode) {
    ...
    return (
        <div className="patient-view">
            ...
            <div className="patient-header" style={{ padding: 'var(--spacing-6) var(--spacing-4)', textAlign: 'center', display: 'block' }}>
                ...
                <div style={{ color: 'var(--color-primary-600)', fontWeight: 'var(--font-weight-semibold)' }}>J-0 • Jour de l'Intervention</div>
            </div>
            ...
            <div className="patient-content fade-in">
                ...
                <button
                    ...
                    disabled={saving}
                >
                    {saving ? 'Enregistrement...' : 'Confirmer ma préparation J-0'}
                </button>
            </div>
