...
import { LogOut, Clock, Scissors, AlertCircle } from 'lucide-react';
...
import { calculateAge, formatDateFR, calculateDaysUntilSurgery, isMilestoneDue } from '../utils/dateUtils';
...
export default function PatientJ7({ patient: propPatient, token: propToken }) {
    ...
    // Check timeline due guard: J-7 is only accessible when isMilestoneDue('J7', patient?.date) is true
    const isDue = isMilestoneDue('J7', patient?.date);
            ...
            <div className="patient-view" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '24px' }}>
                <div className="card" style={{ maxWidth: '500px', textAlign: 'center', padding: '32px', borderRadius: '24px', background: '#f9fafb', border: '1px solid #e5e7eb' }}>
                    ...
                    <p style={{ color: '#4b5563', fontSize: '15px', lineHeight: '1.5', marginBottom: '24px' }}>
                        {t("Ce questionnaire de pré-admission J-7 sera disponible 7 jours avant votre intervention. Tous vos questionnaires dus sont actuellement à jour !")}
                    </p>
                    <button
